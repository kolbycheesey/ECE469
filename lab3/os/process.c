...
void ProcessFreeResources (PCB *pcb) {
  ...
  if (MboxCloseAllByPid(GetPidFromAddress(pcb)) == MBOX_FAIL) {
    printf("FATAL ERROR: could not close mailboxes");
    exitsim();
  }
...
void ProcessRecalcPriority(PCB *pcb) {
  //formula 1 on lab doc
  pcb->priority = BASE_PRIORITY + pcb->estcpu / 4 + 2 * pcb->pnice;
}

//obtains index of the queue array based on the pcb passed in
int WhichQueue(PCB *pcb) {
  int queue = pcb->priority / PRIORITIES_PER_QUEUE;
  return queue;
}

//return 0 if you can successfully insert into the queue, otherwise return 1 for failure
int ProcessInsertRunning(PCB *pcb) {
  Queue *queue = &runQueues[WhichQueue(pcb)]; //queue of linked lists
  if (AQueueInsertLast(queue, pcb->l) == QUEUE_FAIL) {
    printf("Failed to insert in runqueue!\n");
    exitsim();
  }
  return 0;
}
...
PCB * ProcessFindHighestPriorityPCB() {
  ...
  for (i = 0; i < NUM_RUN_QUEUES; i++) {
    if (runQueues[i].nitems) {
      break;
    }
  }

  //return first item of that linked list since that one has the highest priority
  l = AQueueFirst(&runQueues[i]);
  pcb = (PCB *)AQueueObject((Link *)AQueueObject(l));
  return pcb;
}
...
void ProcessDecayAllEstcpus() {
  ...
  Link *iterator;
  for (i = 0; i < PROCESS_MAX_PROCS; i++) {
    iterator = AQueueFirst(&runQueues[i]);
    //loop through queue
    while (iterator) {
      ProcessDecayEstcpu((PCB *)AQueueObject(iterator));
      ProcessRecalcPriority((PCB *)AQueueObject(iterator));
      iterator = AQueueNext(iterator);
    }
  }
}
...
void ProcessFixRunQueues() {
  ...
  PCB *pcb;
  //iterate through runqueues array
  for (i = 0; i < PROCESS_MAX_PROCS; i++) {
    iterator = AQueueFirst(&runQueues[i]);
    //iterate through queue
    while (iterator) {
      pcb = (PCB *)AQueueObject(iterator);
      iterator = AQueueNext(iterator);
      if (AQueueRemove(&pcb->l) == QUEUE_FAIL) {
        printf("Failed to remove from queue\n");
      }
      //create a link for the current pcb to be inserted
      if ((pcb->l = AQueueAllocLink(pcb)) == NULL) {
        printf("Failed to allocate a link for the pcb\n");
      }
...
void BSDScheduling() {
  ...
  Link *iterator;
  pcb = ProcessFindHighestPriorityPCB(); 
  pcb->jiffies += PROCESS_QUANTUM_JIFFIES;
  if (pcb->pinfo) {
    printf(PROCESS_CPUSTATS_FORMAT, (int)(pcb - pcbs), pcb->jiffies, 0);
  }
  currentPCB = pcb;
  queueNo = WhichQueue(pcb);
  queueHead = AQueueFirst(&runQueues[queueNo]);
  //find link

  pcbToReinsert = (Link *)AQueueObject(queueHead);
  newHead = queueHead->next;
  newHead->prev = NULL;
  pcbToReinsert->next = NULL;

  //update priorities for process about to run
  pcb->estcpu++;
  ProcessRecalcPriority(pcb); 
  //reinsert at the end of the queue
  queueTail = AQueueLast(&runQueues[queueNo]);
  iterator = (Link *)AQueueObject(queueHead);
  while (iterator->next) {
    iterator = iterator->next;
  }
  iterator->next = pcbToReinsert;
  pcbToReinsert->prev = iterator;
}
...
void ProcessSchedule () {
  ...
  currentPCB->jiffies += PROCESS_QUANTUM_JIFFIES;
  if (pcb->pinfo) {
    printf(PROCESS_CPUSTATS_FORMAT, (int)(pcb - pcbs), pcb->jiffies, 0);
  }
  if (currentPCB->flags & PROCESS_STATUS_RUNNABLE) {
    rrQueue = &runQueues[WhichQueue(currentPCB)];
    if (AQueueMoveAfter(rrQueue, AQueueLast(rrQueue), AQueueFirst(rrQueue)) == QUEUE_FAIL) {
      printf("Failed to rearrange queue in round robin fashion\n");
    }
    currentPCB->estcpu++;
    //update priority;
  ...
  //update all priorities after 10 cpu quanta
  if (processTimer % 10 == 0) {
...
void ProcessUserWakeup () {
  ...
  PCB *pcb;
  int sleptTime;
  iterator = AQueueFirst(&waitQueue);
  while (iterator) {
    sleptTime = ClkGetCurJiffies() - pcb->stime;
    pcb = AQueueObject(iterator);
    if (sleptTime - pcb->wtime >= 0) { //check if a process is an autowake process
      // Make sure it's not yet a runnable process.
      dbprintf ('p',"Waking up PID %d.\n", (int)(pcb - pcbs));
      ...
      ProcessDecayEstcpuSleep(pcb, sleptTime);
      ProcessRecalcPriority(pcb);
      iterator = AQueueNext(iterator);
      if (AQueueRemove(&(pcb->l)) != QUEUE_SUCCESS) {
        printf("FATAL ERROR: could not remove wakeup PCB from waitQueue in ProcessWakeup!\n");
      ...
      ProcessInsertRunning(pcb);
      //reset wake and sleep times as process gets awoken
      pcb->wtime = 0;
      pcb->stime = 0;
    }
    else {
...
void ProcessUserSleep(int seconds) {
  ...
  currentPCB->stime = ClkGetCurJiffies();
  currentPCB->wtime = seconds * CLOCK_PROCESS_JIFFIES;
  if (AQueueRemove(&currentPCB->l) == QUEUE_FAIL) {
    printf("Failed to remove from queue!\n");
  }
  if ((currentPCB->l = AQueueAllocLink(currentPCB)) == NULL) {
    printf("Failed to obtain alloc link!\n");
  }
  if (AQueueInsertLast(&waitQueue, currentPCB->l) != QUEUE_SUCCESS) {
    printf("Failed to insert into wait Queue!\n");
  }
