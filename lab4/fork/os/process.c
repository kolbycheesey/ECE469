...
void ProcessFreeResources (PCB *pcb) {
  ...
  //loop through pagetable, calculate all page numbers to free, and set all entries to 0
  for (i = 0; i < MEM_PT_SIZE; i++) {
    if (pcb->pagetable[i]) {
      pageNumber = MemoryGetPageNumber(pcb->pagetable[i]);
      MemoryFreePage(pageNumber);
    }
    //  pcb->pagetable[i] = 0;
  }
  pcb->npages = 0;
...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
  ...
  //user stack
  dbprintf('m', "Allocating pages\n");
  pcb->npages = 0;
  if ((userStackPage = MemoryAllocPage()) == -1) {
    printf("Failed to allocate user stack!\n");
    exitsim();
  }
  pcb->npages++;
  referenceCounter[userStackPage] = 1;
  if ((sysStackPage = MemoryAllocPage()) == -1) {
    printf("Failed to allocate system stack!\n");
    exitsim();
  }
  pcb->npages++;
  ...
  for (i = 0; i < 4; i++) {
    dbprintf('m', "Virtual Page Number %d\n", i);
    if ((codePage = MemoryAllocPage()) == -1) {
      printf("Failed to allocate code/data seg!\n");
      exitsim();
    }
    referenceCounter[codePage] = 1;
    pcb->pagetable[i] = MemorySetupPte(codePage);
    pcb->npages++;
  }
  pcb->sysStackArea = MemorySetupPte(sysStackPage) & ~(0x7);

  //initialize system stack pointer
  stackframe = (uint32 *)((pcb->sysStackArea + MEM_ADDRESS_OFFSET_MASK) & ~(0x3));

  //initialize saved frame fields
  ...
  pcb->currentSavedFrame[PROCESS_STACK_PTSIZE] = MEM_PT_SIZE;
  pcb->currentSavedFrame[PROCESS_STACK_PTBITS] = ((MEM_L1FIELD_FIRST_BITNUM) << 16) | (MEM_L1FIELD_FIRST_BITNUM);
  //printf("%x\n", pcb->currentSavedFrame[PROCESS_STACK_PTBITS]);
  ...
  if (isUser) {
    ...
    pcb->pagetable[MemoryGetPageNumber(stackframe[PROCESS_STACK_USER_STACKPOINTER])] = MemorySetupPte(userStackPage);
    ...
    //printf("0x%08x\n", *(uint32 *)(MemoryTranslateUserToSystem(pcb, start)));
...
PCB *ProcessRealFork() {
  ...
  //printf("sysStackPtr: %x\n", (uint32)childPCB->sysStackPtr);
  childPCB->sysStackPtr -= PROCESS_STACK_FRAME_SIZE;
  childPCB->currentSavedFrame = childPCB->sysStackPtr;
  for (i = 11; i <= 80; i++) {
    childPCB->currentSavedFrame[i] = currentPCB->currentSavedFrame[i];
  }
 // printf("IAR child: %x parent: %x\n", childPCB->currentSavedFrame[PROCESS_STACK_IAR], currentPCB->currentSavedFrame[PROCESS_STACK_IAR]);
  childPCB->currentSavedFrame[PROCESS_STACK_PTBASE] = (uint32)&childPCB->pagetable[0];

  for (i = ((MEM_MAX_VIRTUAL_ADDRESS) &~ (MEM_ADDRESS_OFFSET_MASK)); i >= (childPCB->currentSavedFrame[PROCESS_STACK_USER_STACKPOINTER] &~ MEM_ADDRESS_OFFSET_MASK); i -= MEM_PAGESIZE) {
    childPCB->pagetable[MemoryGetPageNumber(i)] |= MEM_PTE_READONLY;
    currentPCB->pagetable[MemoryGetPageNumber(i)] |= MEM_PTE_READONLY;
  //  printf("%d %d\n", childPCB->pagetable[MemoryGetPageNumber(i)], currentPCB->pagetable[MemoryGetPageNumber(i)]);
    referenceCounter[MemoryGetPageNumber(childPCB->pagetable[MemoryGetPageNumber(i)])]++;
   // printf("%d\n", referenceCounter[MemoryGetPageNumber(childPCB->pagetable[MemoryGetPageNumber(i)])]);
  }
  intrs = DisableIntrs ();
...
/*  if (currentPCB == NULL) {
    dbprintf ('p', "Setting childPCB=0x%x, stackframe=0x%x\n",
  ...
  for (i = 0; i < MEM_PT_SIZE; i++) {
    if ((currentPCB->pagetable[i] & 0x1) == 1) {
