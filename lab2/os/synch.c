...
int CondWait(Cond *c) {
  ...
  if (&locks[c->lck]) return INVALID_COND;
  ...
  LockHandleRelease(c->lck);
  if (!AQueueInsertLast(&c->waiting, l)) {
    exitsim();
  }
  ProcessSleep();
  return SYNC_SUCCESS;
...
int CondSignal(Cond *c) {
  ...
  if (locks[c->lck].pid != GetCurrentPid()) {
    return 1;
  }
  intrs = DisableIntrs ();
...
int GetSemCount(sem_t sem) {
  return sems[sem].count;
}
